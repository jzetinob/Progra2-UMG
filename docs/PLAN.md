...
- **Buscador inteligente (commit `...`)**: la decisión 2 (combos) se reemplazó por el componente reutilizable `CampoBusqueda` (autocompletado): se escribe y filtra por código/NIT o nombre, con flechas + Enter o clic para elegir. Motivo: con catálogos grandes, un desplegable no es productivo. Si algún día hay millones de registros, la búsqueda real debe pasar a SQLite con `LIKE`.
